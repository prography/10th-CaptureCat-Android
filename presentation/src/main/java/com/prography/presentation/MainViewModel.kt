    ...
    fun initChecking() {
        viewModelScope.launch {
            ...
            _startDestination.value  = when {
                !isOnboardingShown -> AppRoute.Onboarding.toString() // 처음 앱에 진입하는 경우, 온보딩
                accessToken.isNullOrEmpty() -> AppRoute.Main.toString() // 토큰이 존재하지 않는 경우, 메인 화면
                else -> AppRoute.Main.toString() // 토큰이 존재하는 경우, 메인 화면 : 유효 검사 interceptor 추후 처리
            }
